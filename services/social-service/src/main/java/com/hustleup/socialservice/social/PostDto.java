...
public class PostDto {
    ...
    private static String resolvePrimaryImageUrl(Post post, List<PostMediaDto> media) {
        ...
        return media.stream().filter(item -> "IMAGE".equals(item.getType())).map(PostMediaDto::getUrl).findFirst().orElse(null);
    }
    ...
    private static List<String> splitCsv(String value) {
        ...
        return Arrays.stream(value.split(",")).map(String::trim).filter(entry -> !entry.isBlank()).toList();
    }
