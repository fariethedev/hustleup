...
export default function ListingCard({ listing, index = 0, onDelete }) {
  ...
  const handleAddToCart = (e) => {
    ...
    // Normalized to GBP at add-time so a cart mixing listings/shops that were priced in
    // different currencies still sums to a correct total.
    dispatch(addToCart({ listingId: listing.id, title: listing.title, price: convertToGBP(listing.price, listing.currency || 'GBP'), currency: 'GBP', image: imageUrl, sellerId: listing.sellerId, sellerName: listing.sellerName || 'Seller' }));
