...
import { X, Briefcase, ShoppingBag, AtSign, Check, Loader2, Eye, EyeOff, MapPin } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Checked here as well as inline: the inline hint only appears once the second field has
    // been typed in, and the form is submittable from the first one with Enter.
      ...
      >
        ...
        <div className="glass bg-black/60 border border-white/10 rounded-3xl p-6 shadow-2xl backdrop-blur-3xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {/* Confirm password. A mistyped password on a sign-up form is invisible until the
                person tries to log in and cannot, by which point they have no way to tell a
            ...
            <div>
              ...
              {/* Only once they have actually typed something — flagging a mismatch against an
                  empty box would mark the field red before there is anything to be wrong. */}
              {mismatch && (
                <p className="mt-1.5 text-[11px] text-red-400 font-medium">Both passwords must match</p>
              )}
            </div>

            {/* City. Optional: the marketplace works without it, and a required field here
                would cost sign-ups from anyone whose city is not on the list. Setting it is
                what puts a real location on their listings and shop card instead of the
                country-level "Polska" fallback. */}
            <div>
