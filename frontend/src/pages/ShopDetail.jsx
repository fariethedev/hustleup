  ...
  if (loading) {
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-12 py-10 grid grid-cols-1 md:grid-cols-3 gap-8">
          {[...Array(6)].map((_, i) => (
    ...
    <div className="min-h-screen text-white">
      ...
      <section className="relative h-[260px] sm:h-[320px] overflow-hidden">
        ...
        <div className="absolute top-16 left-0 right-0 px-6 sm:px-12 flex items-center justify-between z-20">
          <Link
            to="/explore/shops"
            className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-black/70 backdrop-blur-md border border-white/15 text-white font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all active:scale-95"
          >
          ...
          <div className="flex gap-2">
            ...
            {isOwner && (
              <Link
                to="/dashboard"
                className="flex items-center gap-2 px-4 h-10 rounded-2xl bg-[#CDFF00] text-black font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-transform active:scale-95"
              >
                <Pencil className="w-3.5 h-3.5" /> Edit shop
              </Link>
            )}
            <button className="w-10 h-10 rounded-2xl bg-black/70 backdrop-blur-md border border-white/15 flex items-center justify-center hover:scale-110 transition-transform">
          ...
          <div className="max-w-7xl mx-auto flex flex-col items-start gap-2">
            {shop.category && (
              <motion.span
                ...
                className="px-4 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-black/70 backdrop-blur-md border border-white/20"
            ...
            >
              ...
              <span className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-[#CDFF00]" /> {displayCity(shop.city)}
              </span>
              {shop.ownerName && (
                <Link to={`/profile/${shop.ownerId}`} className="flex items-center gap-2 hover:text-white transition-colors">
                  <span className="opacity-50">by</span> {shop.ownerName}
                </Link>
              )}
            </motion.div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 py-10">
        <div className="grid lg:grid-cols-[1fr_320px] gap-10 items-start">
          ...
          <div>
            ...
            {categories.length > 2 && (
              <div className="flex flex-wrap gap-2 mb-8 p-2 rounded-3xl bg-white/[0.03] border border-white/5 w-fit">
                {categories.map((cat) => (
                  <button
                    key={cat}
              ...
              {filteredProducts.map((product, i) => (
                ...
                >
                  <Link
                    ...
                    className="group flex flex-col h-full rounded-[32px] overflow-hidden bg-black/60 border border-white/10 hover:border-[#CDFF00]/40 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(0,0,0,0.8)]"
                  >
                  {/* Product photo */}
                  <div
                    className="h-36 shrink-0 relative overflow-hidden bg-black/40 border-b border-white/5"
                  >
                    <SmartImage
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-24 rounded-[48px] border-dashed border-2 border-white/5">
          ...
          <aside className="sticky top-32 flex flex-col gap-8">
             {isOwner && (
               <div className="p-5 rounded-[32px] bg-[#CDFF00]/5 border border-[#CDFF00]/30">
             ...
             {!isOwner && (
               <div className="p-8 rounded-[32px] bg-white/[0.03] border border-white/5 overflow-hidden relative">
                  <div
